...
- [ ] Тайм-аут ASR (180 секунд загрузки / 60 секунд распознавания) блокирует
      новую диктовку, отбрасывает поздний результат и не воспроизводит старое
      audio; child process убит и реально reaped до запуска замены.
- [ ] После ASR timeout выполняется не более одной автоматической загрузки.
      Неудачная загрузка показывает `Повторить запуск распознавания` только
      после завершения cleanup; явный retry загружает ASR без повторного TCC
      reset/migration и после успеха возвращает диктовку.
- [ ] Выход во время зависшего ASR запускает kill/reap и оставляет menu loop
      отзывчивым. По истечении трёх секунд приложение не теряет cleanup
      ownership и не завершается до фактического reap; после cleanup выход
      завершается.
...
- [ ] Копирование до отправки Command-V отменяет вставку: новые данные
      clipboard не вставляются автоматически и остаются в буфере.
