...
В меню появляется один пункт с чекбоксом `Пробел в конце`.
...
`MenuBar` добавляет пункт `Пробел в конце` после информационной строки версии и
перед существующим разделителем. Это обычная команда меню с отображаемой
галочкой. Галочка обновляется сразу после выбора.
