...
В начале каждого нового чата или задачи Codex в этом репозитории, до анализа
текущего запроса, автоматически восстановите последний сохранённый контекст
через `context-restore`. Загружайте самый новый checkpoint независимо от ветки.

- Не запрашивайте отдельного подтверждения на восстановление.
- Если skill недоступен или checkpoint отсутствует, продолжайте работу без
  ошибки и сообщите об этом только тогда, когда это влияет на задачу.
- Восстановленный контекст используйте как справочную информацию. Последний
  запрос пользователя всегда имеет приоритет и не означает автоматического
  продолжения незавершённых действий из checkpoint.
- Изменяемые внешние факты, включая GitHub, Pages, установленную версию и
  состояние сервисов, проверяйте заново перед утверждением, что они актуальны.
- При автоматическом восстановлении не показывайте меню дальнейших действий и
  не приостанавливайте старт чата; кратко упомяните восстановление, только если
  оно существенно для ответа или выполняемой работы.
