...
pub enum HotkeySignal {
    /// Internal FIFO preflight marker; never generated by physical input.
    #[doc(hidden)]
    Boundary(u64),
    Pressed,
    Released {
...
impl CallbackState {
    fn emit_observation(&self, observation: KeyboardObservation) -> GateDecision {
        ...
        if let Some(signal) = decision.signal {
            self.notifier.send(&self.sender, signal);
        }
        decision
    }
    ...
    fn recover_tap(&self, kind: ObservationKind) {
        ...
        self.notifier.send(&self.sender, HotkeySignal::TapLost);
        ...
        unsafe {
            ...
            if cg_event_tap_is_enabled(tap.as_ptr()) {
                self.notifier.send(&self.sender, HotkeySignal::TapRestored);
            }
        }
    }
}
...
impl HotkeyListener {
    pub fn install(
        ...
        Self::install_notified(sender, control, EventNotifier::default())
    }

    pub(crate) fn install_notified(
        sender: Sender<HotkeySignal>,
        control: HotkeyControl,
        notifier: EventNotifier,
    ) -> Result<Self, HotkeyInstallError> {
...
mod tests {
    ...
    #[test]
    fn observation_and_tap_loss_wake_the_source_without_a_tick() {
        ...
        state.recover_tap(ObservationKind::TapDisabledByTimeout);
        crate::event_wake::tests::pump_until(|| signals.borrow().len() == 3);
        assert_eq!(
            *signals.borrow(),
            [
